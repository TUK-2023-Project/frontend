...
import type * as h from "@mediapipe/holistic";
...
function CameraDisplay({ cameraOn }: WebCamProps) {
  ...
  const onResults: h.ResultsListener = (results) => {
    ...
    if (canvasRef.current == null || !webcamRef.current?.video || !cameraOn) {
      return;
    }
  ...
  return (
    <>
      ...
      />
      {cameraOn && <canvas className="canbus_box" ref={canvasRef} />}
    </>
  );
}
